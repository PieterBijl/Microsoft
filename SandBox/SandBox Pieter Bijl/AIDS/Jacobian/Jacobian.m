...
% position and the derivative of quaternions wrt quaternions
    ...
    for i = 1:n
        q = state_vector(7:10);
        xyz = quat2rotm(q')*wireframe(:,i);%[state_vector(1); state_vector(3); state_vector(2)];
        xyz = [xyz(1)+state_vector(1); xyz(3) + state_vector(3); xyz(2) + state_vector(2)];
        H = H_int(fx, fy, xyz(1), xyz(2), xyz(3));
        vectorJacobian(i*2-1:i*2,1:3) = H*eye(3,3);
        H_q = H*dH_dq(q,xyz);
